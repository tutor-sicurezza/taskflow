...
import { LINGUA_PREDEFINITA, type Lingua } from '@/lib/i18n';
...
const STRUTTURE: Record<NotificationType, Struttura> = {
  task_assigned: {
    coloreTitolo: '#2c3e50',
    riquadro: {
      sfondo: '#f8f9fa',
...
interface TestiModelli {
  ...
  apri: string;
  firma: [string, string];
  modelli: Record<NotificationType, VociModello>;
}

const TESTI: Record<Lingua, TestiModelli> = {
  it: {
...
function corpoHtml(tipo: NotificationType, testi: TestiModelli): string {
...
function corpoTesto(tipo: NotificationType, testi: TestiModelli): string {
  const s = STRUTTURE[tipo];
  const v = testi.modelli[tipo];
  const r = s.riquadro;
...
export function tipiNotifica(lingua: Lingua): { value: NotificationType; label: string }[] {
  const testi = TESTI[lingua] ?? TESTI[LINGUA_PREDEFINITA];
  return (Object.keys(STRUTTURE) as NotificationType[]).map((value) => ({
    value,
...
): Omit<EmailTemplate, 'id' | 'lastModifiedAt' | 'lastModifiedBy'> {
  const testi = TESTI[lingua] ?? TESTI[LINGUA_PREDEFINITA];
  return {
...
export function modelliPredefiniti(lingua: Lingua) {
  return (Object.keys(STRUTTURE) as NotificationType[]).map((tipo) =>
